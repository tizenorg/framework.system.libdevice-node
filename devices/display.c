...
#define PROPERTY_MASK			0x0FFF
#define PROPERTY_PROP(val)		((val) & PROPERTY_MASK)
#define PROPERTY_INDEX(val)		((val) >> 16)

#define PROPERTY_LUX(val)		((val) >> 16)
#define PROPERTY_FORCE_MASK		0xF
#define PROPERTY_LUX_FORCE(val)	(((val) >> 12) & PROPERTY_FORCE_MASK)
...
static int display_get_prop(int __prop, int *val)
{
	...
	case PROP_DISPLAY_OVERHEATING_CTR:
		return PLUGIN_GET(backlight_overheating_control)(index, val);
	case PROP_DISPLAY_BRIGHTNESS_BY_LUX:
		return PLUGIN_GET(backlight_brightness_by_lux)(lux, val, force);
	case PROP_DISPLAY_CABC:
		return PLUGIN_GET(lcd_cabc)(index, val);
	...
	case PROP_DISPLAY_IMAGE_ENHANCE_COLOR_BLIND:
		return PLUGIN_GET(image_enhance_color_blind)(val);
	case PROP_DISPLAY_AUTO_SCREEN_TONE:
		return PLUGIN_GET(auto_screen_tone)(index, val);
	case PROP_DISPLAY_HBM_CONTROL:
		return PLUGIN_GET(backlight_hbm_control)(index, val);
	case PROP_DISPLAY_ELVSS_CONTROL:
		return PLUGIN_GET(backlight_elvss_control)(index, val);
	}
...
static int display_set_prop(int __prop, int val)
{
	...
	case PROP_DISPLAY_OVERHEATING_CTR:
		return PLUGIN_SET(backlight_overheating_control)(index, val);
	case PROP_DISPLAY_FRAME_RATE:
		return PLUGIN_SET(display_frame_rate)(val);
	case PROP_DISPLAY_CABC:
		return PLUGIN_SET(lcd_cabc)(index, val);
	case PROP_DISPLAY_IMAGE_ENHANCE_MODE:
	...
	case PROP_DISPLAY_IMAGE_ENHANCE_COLOR_BLIND:
		return PLUGIN_SET(image_enhance_color_blind)((void*)val);
	case PROP_DISPLAY_AUTO_SCREEN_TONE:
		return PLUGIN_SET(auto_screen_tone)(index, val);
	case PROP_DISPLAY_HBM_CONTROL:
		return PLUGIN_SET(backlight_hbm_control)(index, val);
	case PROP_DISPLAY_ELVSS_CONTROL:
		return PLUGIN_SET(backlight_elvss_control)(index, val);
	case PROP_DISPLAY_AUTO_SCREEN_TONE_FORCE:
		return PLUGIN_SET(auto_screen_tone_force)(index, val);
	}
