...
#define PROPERTY_MODE(val)      (((val) >> 16) & PROPERTY_MASK)
...
static int extcon_get_prop(int prop, int *val)
{
	...
	case PROP_EXTCON_MUIC_ADC_ENABLE:
		return PLUGIN_GET(extcon)(MUIC_ADC_ENABLE, val);
	case PROP_EXTCON_USB_ID:
		return PLUGIN_GET(extcon)(USB_ID, val);
	}
...
static int extcon_set_prop(int prop, int val)
{
	...
	case PROP_EXTCON_USB_PATH:
		...
		return r;
	case PROP_EXTCON_MUIC_ADC_ENABLE:
		return PLUGIN_SET(extcon)(MUIC_ADC_ENABLE, val);
	}
